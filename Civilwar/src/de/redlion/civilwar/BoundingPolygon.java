...
public class BoundingPolygon {
	 ...
	 */
	public static Array<Vector2> createGiftWrapConvexHull(Array<Vector2> points) {
		...
		for (int i = 0; i < polygon.size; ++i) {
			...
			if (!(norm0 > 0.0f && norm1 > 0.0f) && (toRemove == null || (polygon.size - toRemove.size) > 3)) {
			...
			if (Math.abs(cross) < tolerance && dot > 0 && (toRemove == null || (polygon.size - toRemove.size) > 3)) {
				if (toRemove == null)
					toRemove = new Array<Integer>();
				toRemove.add(i);
			}
	...
	public static Array<Vector2> createEarClipHull(Array<Vector2> points) {

		FloatArray polygon = new FloatArray(points.size * 2);

		for (Vector2 position : points) {
			polygon.add(position.x);
			polygon.add(position.y);
		}

		ShortArray triangles = new EarClippingTriangulator().computeTriangles(polygon);

		Array<Vector2> triangleOutlines = new Array<Vector2>();
		for (int i = 0; i < triangles.size; i += 3) {
			...
			triangleOutlines.add(new Vector2(ax, ay));
			triangleOutlines.add(new Vector2(bx, by));
			triangleOutlines.add(new Vector2(cx, cy));
			triangleOutlines.add(new Vector2(cx, cy));
			triangleOutlines.add(new Vector2(ax, ay));
		}

		return triangleOutlines;
	}
